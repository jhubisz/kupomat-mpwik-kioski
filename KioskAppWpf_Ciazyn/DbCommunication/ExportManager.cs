...
namespace DbCommunication
{
    public class ExportManager
    {
        ...
        public void ServerSync(KioskConfiguration currenctConfiguration, PrinterPaperStatus currentPaperStatus, ProbeStatus probeStatus)
        {
        ...
        public void ExportProbeStatus(ProbeStatus probeStatus)
        {
            if (probeStatus != null)
            {
                DbServerWrite.UpdateProbeStatus(probeStatus);
            }
        }
        ...
        public void ImportHarmonogramDostawca()
        {
            var rowsToImport = DbServerRead.GetHarmonogramDostawcyDbRowsToExport();
            if (rowsToImport.Count > 0)
            {
                foreach (var rowToImport in rowsToImport)
                {
                    if (rowToImport.ExportAction == ExportAction.Insert)
                        DbServerWrite.HarmonogramDostawcaAdd(rowToImport);
                    else
                        DbServerWrite.HarmonogramDostawcaUpdate(rowToImport);
                }
            }
        }
