...
namespace PrinterCommunication
{
    public class PrinterManager
    {
        ...
        private void Print()
        {
            try
            {
                PrintDoc = BuildMultiAddressContent();
                AutoResetEvent resetEvent = new AutoResetEvent(false);
                PrintDocument doc = new PrintDocument();
                doc.PrintPage += new PrintPageEventHandler(ProvidePrintDocContent);
                doc.EndPrint += new PrintEventHandler((sender, e) =>
        ...
        private void ProvidePrintDocContent(object sender, PrintPageEventArgs e)
        {
            ...
            while (PrintDoc.Lines.Count > 0
                && PrintDoc.Lines.Peek().Type != LineType.PageBreak
                && (PrintDoc.TotalPageHeight < 210 || PrintDoc.Lines.Peek().OffsetY == 0))
            {
            ...
            if (PrintDoc.Lines.Peek().Type == LineType.PageBreak)
            {
                PrintDoc.Lines.Dequeue();
            }
        ...
        private PrintDoc BuildReciptContent()
        {
            ...
            #region powód przerwania zrzutu
            if (!Transaction.FinishedCorrectly && Transaction.FinishReason != "")
            {
                printDoc.Lines.Enqueue(
                new PrintLine()
                {
                    Text = "Powód zakończenia: ",
                    Brush = new SolidBrush(Color.Black),
                    Font = new Font("Consolas", 8),
                    OffsetY = 20,
                    OffsetX = 0
                ...
                var powodLines = BuildMultiLine(Transaction.FinishReason, 0);
                if (powodLines != null && powodLines.Count > 0)
                {
                    foreach (var line in powodLines)
                    {
                        printDoc.Lines.Enqueue(line);
                    }
                }
        ...
        private PrintDoc BuildMultiAddressContent()
        {
            var printDoc = new PrintDoc() { TopMarginSize = 12, BottomMarginSize = 12, Lines = new Queue<PrintLine>() };
            ...
            foreach (CustomerAddress addr in Transaction.CustomerAddresses)
            {
                printDoc = BuildAddressReciptContent(printDoc, addr, Transaction.Cargo.Id, counter++);
                printDoc.Lines.Enqueue(
                    new PrintLine()
                    {
        ...
        private PrintDoc BuildAddressReciptContent(PrintDoc printDoc, CustomerAddress address, int cargoTypeId, int addrCounter)
        {
            Font font = new Font("Consolas", 8);
            String underLine = "------------------------------------------";
            ...
            if (cargoTypeId == 3)
            {
                ...
                printDoc.Lines.Enqueue(
                    new PrintLine()
                    {
                        Text = address.RodName + ", dz. " + address.AddressNumber,
                        Brush = new SolidBrush(Color.Black),
                        Font = new Font("Consolas", 8),
                        OffsetY = 0,
                        OffsetX = 30
                ...
                if (address.ContractNo != null && address.ContractNo != "")
                {
                    printDoc.Lines.Enqueue(
                        new PrintLine()
                        {
                            Text = "Umowa: " + address.ContractNo,
                            Brush = new SolidBrush(Color.Black),
                            Font = new Font("Consolas", 8),
                            OffsetY = 20,
                            OffsetX = 30
                ...
                printDoc.Lines.Enqueue(
                    new PrintLine()
                    {
                        Text = address.ActualAmount.ToString("0.00"),
                        Brush = new SolidBrush(Color.Black),
                        Font = new Font("Consolas", 8),
                        OffsetY = 0,
                        OffsetX = 160
            ...
            else
            {
                ...
                if (!string.IsNullOrEmpty(address.UlicaName))
                {
                    printDoc.Lines.Enqueue(
                        new PrintLine()
                        {
                            Text = address.UlicaName + " " + address.AddressNumber,
                            Brush = new SolidBrush(Color.Black),
                            Font = new Font("Consolas", 8),
                            OffsetY = nextOffset,
                            OffsetX = 30
                ...
                else
                {
                    printDoc.Lines.Enqueue(
                        new PrintLine()
                        {
                            Text = address.MiejscowoscName + " " + address.AddressNumber,
                            Brush = new SolidBrush(Color.Black),
                            Font = new Font("Consolas", 8),
                            OffsetY = nextOffset,
                            OffsetX = 30
