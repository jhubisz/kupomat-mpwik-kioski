...
namespace KioskAppWpf.Pages
{
    ...
    /// </summary>
    public partial class SelectCompanyByName : Page
    {
        ...
        private void BindCustomers()
        {
            ...
            {
                Companies = ((MainWindow)Application.Current.MainWindow).DbRead.GetAllRegularCompanies(Transaction.Card.Customer.Id);
                acbCompany.ItemsSource = Companies;
            }
            else
            {
                Companies = ((MainWindow)Application.Current.MainWindow).DbRead.GetAllCompanies();
                acbCompany.ItemsSource = Companies;
            }
        }
        
        private void SelectCompanyByNamePage_KeyUp(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Tab)
            {
                UpdatePage();
            ...
            if (!(Keyboard.FocusedElement is TextBox))
            {
                ...
                if (e.Key == Key.D2)
                {
                    ...
                    if (dbType == AddressDbType.Teryt)
                    {
                        ((MainWindow)Application.Current.MainWindow).UpdateTransactionDbType(AddressDbType.ManualEntry);
                    }
                    if (dbType == AddressDbType.ManualEntry)
                    {
                        ((MainWindow)Application.Current.MainWindow).UpdateTransactionDbType(AddressDbType.Teryt);
        ...
        private void PageLoaded(object sender, RoutedEventArgs e)
        {
            ...
            Application.Current.MainWindow.KeyUp -= new KeyEventHandler(SelectCompanyByNamePage_KeyUp);
            Application.Current.MainWindow.KeyUp += new KeyEventHandler(SelectCompanyByNamePage_KeyUp);
        }
