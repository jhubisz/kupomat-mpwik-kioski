...
namespace OpcCommunication.Devices
{
    public class Flow
    {
        ...
        public decimal FlowPressureCurr { get; set; }
        public decimal FlowPressureMin { get; set; }
        public decimal FlowPressureMed { get; set; }
        public decimal FlowPressureMax { get; set; }
        ...
        public void CheckStatus()
        {
            if (OpcOn)
            {
                try
                {
                    ...
                    FlowFlowCurr =  Decimal.Divide(int.Parse(stringArray[11]), 100); 
                    ...
                    FlowPressureCurr = Decimal.Divide(int.Parse(stringArray[16]), 100);
                    FlowPressureMin = Decimal.Divide(int.Parse(stringArray[17]), 100);
                    FlowPressureMed = Decimal.Divide(int.Parse(stringArray[18]), 100);
                    FlowPressureMax = Decimal.Divide(int.Parse(stringArray[19]), 100);
        ...
        public override bool Equals(object obj)
        {
            if (obj is Flow)
            {
                return FlowpHCurr == ((Flow)obj).FlowpHCurr 
                    ...
                    && FlowPressureCurr == ((Flow)obj).FlowPressureCurr
                    && FlowPressureMin == ((Flow)obj).FlowPressureMin
                    && FlowPressureMed == ((Flow)obj).FlowPressureMed
                    && FlowPressureMax == ((Flow)obj).FlowPressureMax
