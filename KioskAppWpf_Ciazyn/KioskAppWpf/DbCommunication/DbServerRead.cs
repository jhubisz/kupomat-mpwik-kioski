...
namespace DbCommunication
{
    public class DbServerRead
    {
        ...
        public List<CargoTypeDbRow> GetCargoTypeRowToExport()
        {
            var list = new List<CargoTypeDbRow>();

            var parameters = new List<SqlParameter>
            {
                new SqlParameter("@rowTypeId", (int)ExportRowType.CargoType),
                new SqlParameter("@kioskId", connection.KioskId)
            };
            string query = "SELECT s.Id, s.Nazwa, s.DataStart, s.DataKoniec, s.Deleted "
                         + "    , er.Id as ExportRowId, er.RowActionId as ExportActionId "
                         + "FROM dbo.[RodzajSciekow] s "
            ...
            foreach (DataRow row in result.Rows)
            {
                var item = new CargoTypeDbRow()
                {
                    ...
                    DateStart = (DateTime)row["DataStart"],
                    Deleted = (bool)row["Deleted"],
                };
                ...
                if (row["DataKoniec"] is DBNull)
                    ...
                    item.DateEnd = (DateTime)row["DataKoniec"];
        ...
        public List<KartaDbRow> GetKartaDbRowsToExport()
        {
            ...
            string query = "SELECT k.Id, k.KartaId, k.SamochodId, k.KartaWymianyTacki, k.Blokada, k.DataStart, k.DataKoniec, k.Deleted "
                         + "    , er.Id as ExportRowId, er.RowActionId as ExportActionId "
                         + "    , k.rodzajSciekowId, k.firmaId, k.firmaAdresId, k.gminaId, k.miejscowoscId, k.ulicaId, k.numerId "
                         + "FROM dbo.[Karta] k "
            ...
            foreach (DataRow row in result.Rows)
            {
                ...
                //if (row["RodzajSciekowId"] is DBNull)
                //{
                //    card.RodzajSciekowId = null;
                //}
                //else
                //{
                //    card.RodzajSciekowId = (int)row["RodzajSciekowId"];
                //}

                //if (row["FirmaId"] is DBNull)
                //{
                //    card.FirmaId = null;
                //    card.FirmaAdresId = null;
                //}
                //else
                //{
                //    card.FirmaId = (int)row["FirmaId"];
                //    card.FirmaAdresId = (int)row["FirmaAdresId"];
                //}

                //if (row["GminaId"] is DBNull)
                //{
                //    card.GminaId = null;
                //}
                //else
                //{
                //    card.GminaId = (int)row["GminaId"];
                //}

                //if (row["MiejscowoscId"] is DBNull)
                //{
                //    card.MiejscowoscId = null;
                //}
                //else
                //{
                //    card.MiejscowoscId = (int)row["MiejscowoscId"];
                //}
                ...
                //if (row["NumerId"] is DBNull)
                //{
                //    card.NumerId = null;
                //}
                //else
                //{
                //    card.NumerId = (long)row["NumerId"];
                //}
        ...
        //public List<NumerDbRow> GetNumerDbRowsToExport()
        //{
        //    var list = new List<NumerDbRow>();

        //    var parameters = new List<SqlParameter>
        //    {
        //        new SqlParameter("@rowTypeId", (int)ExportRowType.Numer),
        //        new SqlParameter("@kioskId", connection.KioskId)
        //    };
        //    string query = "SELECT n.Id, n.MiejscowoscId, n.UlicaId, n.Numer, n.DataStart, n.DataKoniec, n.Deleted "
        //                 + "    , er.Id as ExportRowId, er.RowActionId as ExportActionId "
        //                 + "FROM dbo.[Numer] n "
        //                 + "    INNER JOIN dbo.[ExportRow] er ON n.Id = er.RowId AND er.RowTypeId = @rowTypeId AND er.KioskId = @kioskId "
        //                 + "WHERE NOT EXISTS (SELECT 1 FROM dbo.ExportRowExported ee WHERE ee.ExportRowId = er.Id AND ee.KioskId = @kioskId);";
        //    var result = connection.GetTable(query, parameters);
        ...
        //            numer.UlicaId = null;
        //        else
        //            numer.UlicaId = (int)row["UlicaId"];
        //        if (row["DataKoniec"] is DBNull)
        //            numer.DataKoniec = null;
        //        else
        //            numer.DataKoniec = (DateTime)row["DataKoniec"];
        ...
        //public List<AglomeracjaAdresDbRow> GetAglomeracjaAddressesDbRowsToImport()
        //{
        //    List<AglomeracjaAdresDbRow> list = new List<AglomeracjaAdresDbRow>();

        //    var parameters = new List<SqlParameter>();
        //    parameters.Add(new SqlParameter("@rowTypeId", (int)ExportRowType.AglomeracjaAdres));
        //    parameters.Add(new SqlParameter("@kioskId", connection.KioskId));
        //    string query = "SELECT a.Id, Gmina, GminaId, Miejscowosc, MiejscowoscId, Ulica, UlicaId, Numer, WAglomeracji, Aglomeracja, DataStart, DataKoniec "
        //                 + "    , er.Id as ExportRowId, er.RowActionId as ExportActionId "
        //                 + "FROM dbo.[AglomeracjaAdresy] a "
        //                 + "    INNER JOIN dbo.[ExportRow] er ON a.Id = er.RowId AND er.RowTypeId = @rowTypeId AND er.KioskId = @kioskId "
        //                 + "WHERE NOT EXISTS (SELECT 1 FROM dbo.ExportRowExported ee WHERE ee.ExportRowId = er.Id AND ee.KioskId = @kioskId);";
        //    var result = connection.GetTable(query, parameters);
        ...
        //    foreach (DataRow row in result.Rows)
        //    {
        //        var address = new AglomeracjaAdresDbRow()
        //        {
        //            ExportRowId = (int)row["ExportRowId"],
        //            ExportRowType = ExportRowType.Customer,
        //            ExportAction = (ExportAction)row["ExportActionId"],
        ...
        //            Numer = row["Numer"].ToString(),
        //            WAglomeracji = (bool)row["WAglomeracji"],
        //            Aglomeracja = row["Aglomeracja"].ToString(),
        //            DataStart = (DateTime)row["DataStart"],
        //        };
        ...
        //        }
        //        if (row["DataKoniec"] is DBNull)
        //            address.DataKoniec = null;
        //        else
        //            address.DataKoniec = (DateTime)row["DataKoniec"];
