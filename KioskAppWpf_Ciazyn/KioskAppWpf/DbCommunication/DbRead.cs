...
namespace DbCommunication
{
    public class DbRead
    {
        ...
        public List<TransactionAddressDbRow> GetTransactionAddressesDbRowsToExport()
        {
            ...
            var parameters = new List<SqlParameter>();
            string query = "SELECT Id, KioskId, TransakcjaId, ZadeklarowanaIlosc, ZlanaIlosc, NumerUmowy, AdresId, FirmaId, RodId, RodNrDzialki "
            ...
            foreach (DataRow row in result.Rows)
            {
                ...
                if (row["ZlanaIlosc"].ToString() != "")
                {
                    transactionAddress.ZlanaIlosc = (decimal)row["ZlanaIlosc"];
                }
        ...
        public List<TransactionParametersDbRow> GetTransactionParametersDbRowsToExport()
        {
            ...
            var parameters = new List<SqlParameter>();
            string query = "SELECT Id, KioskId, TransakcjaId, PhMin, PhMax, PhAvg, PrzewodnoscMin, PrzewodnoscMax, PrzewodnoscAvg, TempMin, TempMax, TempAvg, ChztMin, ChztMax, ChztAvg, Cisnienie "
            ...
            foreach (DataRow row in result.Rows)
            {
                ...
                if (row["Cisnienie"].ToString() != "")
                    transactionParameter.Cisnienie = Int32.Parse(row["Cisnienie"].ToString());
        ...
        private HarmonogramedSample CheckHarmonogramedSampleByCompany(RfidCard card)
        {
            ...
            string query = "SELECT h.Id "
                         ...
                         + "WHERE DATEADD(ms, ISNULL(GodzinaOd, 0) * 1000 * 60, CAST([Data] AS datetime)) < GETDATE() "
                         + "    AND hd.Deleted = 0 "
