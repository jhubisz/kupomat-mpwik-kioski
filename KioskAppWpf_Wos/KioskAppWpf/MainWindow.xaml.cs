...
namespace KioskAppWpf
{
    ...
    public partial class MainWindow : Window
    {
                ...
                try
                {
                    ...
                    if (counter >= 2)
                    {
                        if (opcLive)
                        {
                            ...
                            if (opcState.Flow.DumpStartTime.HasValue && ((!opcState.Flow.SeqAckTime.HasValue && opcState.Flow.SeqAck) || !_opcOn))
                            {
        ...
        private void SequenceFinished()
        {
            ...
            Transaction.EndTotalAmount = OpcManager.GetTotalFlowValue();
        ...
        public bool StartNewTransaction(RfidCard card, StationLicense license)
        {
            ...
            var transaction = new Transaction() { Card = card, Address = card.Address, TransactionStart = DateTime.Now, License = license, TransactionType = TransactionType.Regular };
            TransactionLifeBitLastStateChange = DateTime.Now;
            Transaction = transaction;
            Transaction.StartTotalAmount = OpcManager.GetTotalFlowValue();
        ...
        {
            Transaction.CustomerAddresses.Add(CurrentTransactionAddress);
            if (Transaction.TransactionType != TransactionType.ToiToi)
                CheckHarmonogramedSample(CurrentTransactionAddress);
            PrevTransactionAddress = CurrentTransactionAddress;
            CurrentTransactionAddress = null;
            CurrentTransactionCompany = null;
