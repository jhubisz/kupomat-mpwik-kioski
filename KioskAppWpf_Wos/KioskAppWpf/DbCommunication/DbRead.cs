...
namespace DbCommunication
{
    public class DbRead
    {
        ...
        public List<CargoType> GetCargoTypes()
        {
            ...
            string query = "SELECT rs.Id, rs.Nazwa FROM dbo.RodzajSciekow rs WHERE rs.Deleted = 0 AND ISNULL(rs.KioskId, @kioskId) = @kioskId;";
        ...
        public RfidCard GetRfidCard(string cardId)
        {
            ...
            string query = "SELECT k.Id, k.KartaId, k.SamochodId, k.Blokada as BlokadaKarty, KartaWymianyTacki, KartaToiToi, PojemnoscToiToi "
                         + "FROM dbo.Karta k "
            ...
            var cardType = RfidCardType.Customer;
            if ((bool)row["KartaWymianyTacki"])
                cardType = RfidCardType.Superuser;
            if ((bool)row["KartaToiToi"])
                cardType = RfidCardType.ToiToi;

            if (cardType == RfidCardType.Customer || cardType == RfidCardType.ToiToi)
            {
                ...
                card = new RfidCard()
                {
                    ...
                    Pojemnosc = (decimal)row["PojemnoscToiToi"]
                };
            }
            else
            {
        ...
        public List<TransactionDbRow> GetTransactionDbRowsToExport()
        {
            ...
            string query = "SELECT Id, KioskId, RodzajSciekowId, KlientId, SamochodId, KartaId, NumerUmowy, ZadeklarowanaIlosc, PrzeplywomierzIloscStart, PrzeplywomierzIloscKoniec "
                         + "    , ZlanaIlosc, TransakcjaStart, ZrzutStart, ZrzutKoniec, TransakcjaKoniec "
            ...
            foreach (DataRow row in result.Rows)
            {
                var transaction = new TransactionDbRow()
                {
                    ...
                    PrzeplywomierzIloscStart = row["PrzeplywomierzIloscStart"] != DBNull.Value ? (decimal)row["PrzeplywomierzIloscStart"] : 0m,
                    PrzeplywomierzIloscKoniec = row["PrzeplywomierzIloscKoniec"] != DBNull.Value ? (decimal)row["PrzeplywomierzIloscKoniec"] : 0m,
        ...
        public List<Miejscowosc> GetAllMiasta(int gminaId)
        {
            ...
            string query = "SELECT m.Id, m.Nazwa, [dbo].[fUsunPolskieZnaki](m.Nazwa) as NazwaBezPl, g.Nazwa as Gmina "
                         ...
                         + "WHERE m.GminaId = @gminaId AND m.Deleted = 0;";
            var result = connection.GetTable(query, parameters);
        ...
        public List<Miejscowosc> GetAllRegularMiasta(int gminaId, int customerId)
        {
            ...
            string query = "SELECT DISTINCT m.Id, m.Nazwa, [dbo].[fUsunPolskieZnaki](m.Nazwa) as NazwaBezPl, g.Nazwa as Gmina "
                         ...
                         + "    AND sk.KlientId = @customerId AND m.Deleted = 0;";
            var result = connection.GetTable(query, parameters);
        ...
        public List<Ulica> GetAllUlice(int miejscowoscId)
        {
            ...
            string query = "SELECT u.Id, u.Nazwa, [dbo].[fUsunPolskieZnaki](u.Nazwa) as NazwaBezPl, m.Nazwa as Miejscowosc "
                         ...
                         + "WHERE u.MiejscowoscId = @miejscowoscId AND u.Deleted = 0;";
            var result = connection.GetTable(query, parameters);
        ...
        public List<Ulica> GetAllRegularUlice(int miejscowoscId, int customerId)
        {
            ...
            string query = "SELECT DISTINCT u.Id, u.Nazwa, [dbo].[fUsunPolskieZnaki](u.Nazwa) as NazwaBezPl, m.Nazwa as Miejscowosc "
                         ...
                         + "    AND sk.KlientId = @customerId AND u.Deleted = 0;";
            var result = connection.GetTable(query, parameters);
        ...
        public List<Company> GetAllCompanies()
        {
            ...
            string query = "SELECT f.Id, f.Nazwa "
                         ...
                         + "FROM Firma f "
                         + "    INNER JOIN FirmaAdres fa ON f.Id = fa.FirmaId AND fa.Deleted = 0 "
        ...
        //{
        //    var harmonogramType = HarmonogramType.Company;
        ...
        public bool FlowMeterReadingExistsForDate(DateTime date)
        {
            var parameters = new List<SqlParameter>();
            parameters.Add(new SqlParameter("@kioskId", connection.KioskId));
            parameters.Add(new SqlParameter("@data", date.Date));
            string query = "SELECT COUNT(*) FROM dbo.KioskPrzeplywomierz "
                         + "WHERE KioskId = @kioskId AND Data = @data;";
            var result = connection.GetScalar(query, parameters);
        ...
        public List<KioskPrzeplywomierzDbRow> GetFlowMeterReadingsToExport()
        {
            List<KioskPrzeplywomierzDbRow> list = new List<KioskPrzeplywomierzDbRow>();
            var parameters = new List<SqlParameter>();
            parameters.Add(new SqlParameter("@kioskId", connection.KioskId));
            string query = "SELECT Id, KioskId, Data, Odczyt, DataWpisu "
                         + "FROM dbo.[KioskPrzeplywomierz] kp "
                         + "WHERE kp.KioskId = @kioskId "
                         + "AND NOT EXISTS (SELECT 1 FROM dbo.KioskPrzeplywomierzExport kpe WHERE kpe.KioskPrzeplywomierzId = kp.Id);";
            var result = connection.GetTable(query, parameters);
            ...
            foreach (DataRow row in result.Rows)
            {
                var flowMeterReading = new KioskPrzeplywomierzDbRow()
                {
                    Id = (int)row["Id"],
                    KioskId = (int)row["KioskId"],
                    Data = (DateTime)row["Data"],
                    Odczyt = (int)row["Odczyt"],
                    DataWpisu = (DateTime)row["DataWpisu"]
                };

                list.Add(flowMeterReading);
            }
