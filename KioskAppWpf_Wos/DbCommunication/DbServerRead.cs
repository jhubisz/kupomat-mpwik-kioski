...
namespace DbCommunication
{
    public class DbServerRead
    {
        ...
        public List<KartaDbRow> GetKartaDbRowsToExport()
        {
            ...
            parameters.Add(new SqlParameter("@kioskId", connection.KioskId));
            string query = "SELECT k.Id, k.KartaId, k.SamochodId, k.KartaWymianyTacki, k.KartaToiToi, k.PojemnoscToiToi, k.Blokada, k.DataStart, k.DataKoniec, k.Deleted "
                         + "    , er.Id as ExportRowId, er.RowActionId as ExportActionId "
                         + "FROM dbo.[Karta] k "
            ...
            foreach (DataRow row in result.Rows)
            {
                var card = new KartaDbRow()
                {
                    ...
                    KartaToiToi = (bool)row["KartaToiToi"],
                    PojemnoscToiToi = (decimal)row["PojemnoscToiToi"],
                    DataStart = (DateTime)row["DataStart"],
                    Deleted = (bool)row["Deleted"],
                };
        ...
        public List<CargoTypeDbRow> GetCargoTypeDbRowsToExport()
        {
            ...
            parameters.Add(new SqlParameter("@kioskId", 1));
            string query = "SELECT c.Id, c.Nazwa, c.DataStart, c.DataKoniec, c.Deleted, c.KioskId "
                         + "    , er.Id as ExportRowId, er.RowActionId as ExportActionId "
                         + "FROM dbo.RodzajSciekow c "
            ...
            foreach (DataRow row in result.Rows)
            {
                ...
                if (row["KioskId"] is DBNull)
                    customer.KioskId = null;
                else
                    customer.KioskId = (int)row["KioskId"];
