...
namespace DbCommunication
{
    public class DbConnection : IDbConnection
    {
        ...
        public long InsertRowLong(string query, List<SqlParameter> parameters)
        {
            return InsertRowLong(query, parameters, null, Connection);
        }
        public long InsertRowLong(string query, List<SqlParameter> parameters, SqlTransaction transaction)
        {
            return InsertRowLong(query, parameters, transaction, Connection);
        }
        public long InsertRowLong(string query, List<SqlParameter> parameters, SqlTransaction transaction, SqlConnection connection)
        {
            SqlCommand command = new SqlCommand(query, connection);
            if (parameters != null && parameters.Count > 0)
            {
            ...
            try
            {
                ...
                if (result is int)
                    resultInt = Convert.ToInt64((int)result);
                else
