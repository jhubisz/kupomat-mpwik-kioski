...
namespace KioskAppWpf
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private void RfidCardAction(string cardId, RfidCard card, ReaderLocation location)
        {
            ...
            if (card == null)
            {
                LogDebug($"Akcja kartą RFiD: null");
            }
            else
            {
                LogDebug($"Akcja kartą RFiD: {card.CardId}, {card.Type}, {location}");
            }

            if (KioskConf.Open == KioskOpenState.Closed && (card == null || card.Type != RfidCardType.Superuser || location != ReaderLocation.Toaleta))
            {
                return;
            }
        ...
        private bool CheckNormalCardSequence(ReaderLocation location, RfidCard card)
        {
            ...
            if (location == ReaderLocation.Camera)
            {
                LogDebug("Otwieranie bramy z kamery");
                SendOpenGateSignal();
            }
            if (location == ReaderLocation.GateIn || location == ReaderLocation.GateOut)
            {
                LogDebug("Otwieranie bramy z czytnika");
                SendOpenGateSignal();
            }
            ...
            while (true)
            {
                try
                {
                    ...
                    if (counter >= 2)
                    {
                        ...
                        if (ShowingPage == DisplayPage.ErrorPage && !ErrorState)
                        {
        ...
        private void UpdateOpcParameters()
        {
            ...
            if (OpcManager.OpcOn && OpcManager.OpcParameters != null)
            {
                ...
                //LogDebug($"ProbeScheduled: {OpcManager.OpcParameters.ProbeScheduled}, ProbePermit: {OpcManager.OpcParameters.ProbePermit}, Declared: {OpcManager.OpcParameters.DeclaredSewageAmount}");
        ...
        private void ShowPage(DisplayPage page)
        {
            ...
            if ((ErrorState && !KioskBlocked) && page != DisplayPage.Service && page != DisplayPage.ProbeService)
            {
        ...
        {
            if (ErrorPage.TransactionSaving) return;
        ...
        private void SetDumpPathMessage()
        {
            ...
            var dumpPathId = deviceStateId == 2 || deviceStateId == 3 ? DumpPath.Backup : DumpPath.Regular;
            switch (dumpPathId)
            {
